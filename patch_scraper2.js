...
js = js.replace(/async function scrapeLinkedInStats\(\) \{[\s\S]*?\}\s*catch\s*\(error\)\s*\{\s*return\s*\{\s*error:\s*error\.message\s*\}\s*;\s*\}\s*\}/, newScraper);

fs.writeFileSync(jsPath, js);
console.log('Scraper patched 2');
